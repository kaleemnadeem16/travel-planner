...
Our PostgreSQL database architecture supports comprehensive tracking, versioning, and monitoring for a multi-agent travel planning system with the following requirements:
...
```sql
-- Migration: Add new agent tracking fields
ALTER TABLE agent_executions 
ADD COLUMN context_window_tokens INTEGER DEFAULT 0,
ADD COLUMN context_efficiency_score DECIMAL(4, 2) DEFAULT 0.0;

-- Update existing records with default values
UPDATE agent_executions 
SET context_window_tokens = total_tokens * 0.7
